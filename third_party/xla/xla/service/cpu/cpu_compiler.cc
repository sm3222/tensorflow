...
absl::StatusOr<std::unique_ptr<HloModule>> CpuCompiler::RunHloPasses(
...

  {
    ...
    auto target_machine_options_proto = GetDefaultHostTargetMachineOptions();
    if (options.target_config &&
        options.target_config->cpu_target_machine_options_proto) {
      target_machine_options_proto =
          options.target_config->cpu_target_machine_options_proto.value();
    }
